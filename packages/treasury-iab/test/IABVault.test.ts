...
describe("IABVault", function () {
  ...
  async function deployFixture() {
    ...
    // Deploy a PublicGoodsPool that accepts USDC (for allocation test)
    const PublicGoodsPool = await ethers.getContractFactory("PublicGoodsPool");
    const publicGoodsPool = await PublicGoodsPool.deploy(
      await usdc.getAddress(),
      HEALTH_THRESHOLD,
      INCOME_THRESHOLD,
      parseUSDC(100)
  ...
  describe("Allocate to Prize", function () {
    it("allocates all funds to PublicGoodsPool when threshold met", async function () {
      const { vault, usdc, aavePool, publicGoodsPool, depositor1 } =
        await loadFixture(deployFixture);
      ...
      await vault.setPublicGoodsPool(await publicGoodsPool.getAddress());
      ...
      expect(await usdc.balanceOf(await publicGoodsPool.getAddress())).to.equal(
        parseUSDC(12_000)
      );
    ...
    it("IAB tokens survive allocation (for voting power)", async function () {
      const { vault, publicGoodsPool, depositor1 } =
        await loadFixture(deployFixture);
    ...
    it("rejects allocation without public goods pool set", async function () {
      const { vault, depositor1 } = await loadFixture(deployFixture);
  ...
  describe("Admin", function () {
    it("sets public goods pool address", async function () {
      const { vault, publicGoodsPool } = await loadFixture(deployFixture);

      await expect(vault.setPublicGoodsPool(await publicGoodsPool.getAddress()))
        .to.emit(vault, "PublicGoodsPoolSet")
        .withArgs(await publicGoodsPool.getAddress());

      expect(await vault.publicGoodsPool()).to.equal(await publicGoodsPool.getAddress());
    });

    it("rejects zero address for public goods pool", async function () {
      const { vault } = await loadFixture(deployFixture);

      await expect(
        vault.setPublicGoodsPool(ethers.ZeroAddress)
      ).to.be.revertedWith("IABVault: zero public goods pool");
    });

    it("only owner can set public goods pool", async function () {
      const { vault, publicGoodsPool, depositor1 } =
        await loadFixture(deployFixture);
      ...
      await expect(
        vault
          .connect(depositor1)
          .setPublicGoodsPool(await publicGoodsPool.getAddress())
      ).to.be.revertedWithCustomError(vault, "OwnableUnauthorizedAccount");
    });
  });
  ...
  describe("Full lifecycle — success path", function () {
    it("deposit → threshold met → allocate → IAB tokens remain for voting", async function () {
      const { vault, usdc, aavePool, publicGoodsPool, depositor1, depositor2 } =
        await loadFixture(deployFixture);
      ...
      const publicGoodsPoolBal = await usdc.balanceOf(await publicGoodsPool.getAddress());
      expect(publicGoodsPoolBal).to.equal(parseUSDC(15_000));
