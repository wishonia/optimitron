...
describe('optimize earth prompt helpers', () => {
  it('publishes the DB-first protocol steps', () => {
    expect(OPTIMIZE_EARTH_PROTOCOL_STEPS).toContain('Call getNextTask with your capabilities.');
  ...
  it('builds an instruction that enforces lease-based work', () => {
    ...
    expect(instruction).toContain('Optimize earth using the task database via MCP as the source of truth.');
    expect(instruction).toContain('Advertise these capabilities when calling getNextTask: typescript, policy analysis.');
  ...
  it('supports custom task source labels and parallelism caps', () => {
    ...
    expect(instruction).toContain('Use your actual capabilities when calling getNextTask.');
  });
});
