...
function generateAggregateReport(results: CountryResult[], agg: AggregateResult): string {
  ...
  lines.push('');
  lines.push(`> Generated ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} by @optomitron/optimizer`);
  lines.push('');
  ...
  lines.push('*Analysis pipeline: @optomitron/optimizer → runFullAnalysis() with temporal alignment, onset delays, change-from-baseline, Bradford Hill scoring*');
...
async function main() {
  ...
  const jsonOutput = {
    ...
    generatedBy: '@optomitron/optimizer (N-of-1 causal analysis)',
