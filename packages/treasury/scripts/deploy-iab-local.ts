...
/**
 * Deploy IABVault + PrizePool + mocks to local Hardhat node.
...
async function main() {
  ...
  // --- 6. Wire IABVault → PrizePool ---
  await vault.setPrizePool(poolAddress);
  console.log("\n6. IABVault.prizePool =", poolAddress);
  ...
  // --- 8. Simulate some initial yield (makes it more interesting) ---
  // Skip this — let users deposit first, then we can simulate yield
