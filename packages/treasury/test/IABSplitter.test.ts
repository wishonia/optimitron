...
describe("IABSplitter", function () {
  async function deployFixture() {
    const [owner, prizePool, investorPool, politicalAllocator, nonOwner] =
  ...
  describe("Deployment", function () {
    ...
    it("sets correct recipients", async function () {
      const { splitter, prizePool, investorPool, politicalAllocator } =
        await loadFixture(deployFixture);
      expect(await splitter.prizePool()).to.equal(prizePool.address);
    ...
    it("rejects zero recipient addresses", async function () {
      ...
      ).to.be.revertedWith("Splitter: zero prize pool");
    });
  });

  describe("split", function () {
    it("splits funds 80/10/10", async function () {
      const { usdc, splitter, prizePool, investorPool, politicalAllocator } =
        await loadFixture(deployFixture);
      ...
      expect(await usdc.balanceOf(prizePool.address)).to.equal(expectedPrize);
    ...
    it("handles multiple splits correctly", async function () {
      const { usdc, splitter, prizePool } = await loadFixture(deployFixture);
      ...
      expect(await usdc.balanceOf(prizePool.address)).to.equal(totalExpected);
    });
  });
  ...
  describe("setRecipients", function () {
    it("allows owner to update recipients", async function () {
      ...
      expect(await splitter.prizePool()).to.equal(newPrize.address);
    });

    it("rejects non-owner", async function () {
      const { splitter, nonOwner, prizePool, investorPool, politicalAllocator } =
        await loadFixture(deployFixture);
      await expect(
        splitter
          .connect(nonOwner)
