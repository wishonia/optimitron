...
- [x] Efficiency analysis — `analyzeEfficiency()` combining efficient frontier + minimum effective spending + overspend ratio (`packages/obg/src/efficiency-analysis.ts`)
- [x] Legislation drafter — Gemini + Google Search grounding for drafting model bills with citations (`packages/agent/src/legislation-drafter.ts`)
- [x] Policy evaluation schedule — sunset clauses, biennial review, hard sunset on welfare metrics (`packages/opg/src/policy-evaluation-schedule.ts`)
- [x] Static analysis site — 48 cross-linked markdown pages served by 11ty (`reports/site/`, generated by `generate-site-reports.ts`)
